...
export class animePaheSevice extends baseSevice {
    constructor() {
        super('https://api.consumet.org/anime/animepahe/')
    }
    ...
    async getAnimeInfoById(id) {
        let anime = await this.get("info/" + id)
        return new animeInfoData(anime);
    }
    async getStremLink(id) {
        let animelinks = await this.get("watch/" + id)
        let links = {
